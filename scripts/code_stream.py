...
# Minified / autogenerated code is not worth learning token order from — it is
# machine-emitted, one-line-per-file bundles whose adjacencies are noise. Flag it
# with the BigCode/The-Stack heuristic: a very long single line or a high average
# line length. (Also protects the lexer from pathological single-line inputs.)
_MAX_LINE_LEN = 1000
_MEAN_LINE_LEN = 100
...
def is_minified(text):
    ...
    return max_len > _MAX_LINE_LEN or mean_len > _MEAN_LINE_LEN


def file_streams(paths, split_identifiers=False, skip_minified=True):
    for path in paths:
        kept = skipped = 0
        with open(path, "r", encoding="utf-8", errors="replace") as handle:
            for line_number, line in enumerate(handle):
                ...
                if not content:
                    continue
                if skip_minified and is_minified(content):
                    skipped += 1
                    continue
                kept += 1
                yield str(line_number), tokenize_code(content, split_identifiers=split_identifiers)
        if skip_minified and skipped:
            print(f"[code_stream] {path}: kept {kept}, skipped {skipped} minified", flush=True)
